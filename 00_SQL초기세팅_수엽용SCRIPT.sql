...
댜표적으로 
SELEECT VESION(); CTRL+ENTER(한 줄 실행) 으로 버전이 무사히 잘 나오는 잘 동작 하는것
...
-- CREATE DATABASE  : 데이터 베이슬 생성할 것인데 
-- IF NOT EXISTS : 만약에 오른쪽에 작성한 명칭의 데이터베이스가 존재 하지 않는다면 
-- CREATE DATABASE IF NOT EXISTS 0000000; 0000000 의 이름의 데이터베이스를 생성 
-- 한 줄 작성 완료 ; : 반드시 맨 뒤에 붙여야 함

USE employee_management;
-- USE 0000000; : 개발자나 관리자가 사용할 데이터베이스 명칭을 선택해서 사용
-- 이후 실핼될 모든 SQL 명령어는 0000000 데이터베이스에서 동작하고 작동하도록 설정 
-- PM 작급 이나 팀장급 정도의 사람들만 사용하는 명령어
