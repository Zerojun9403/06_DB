...
customer_name, email, password, phone ,address, create_at
`store_id` 
  `store_name`
  `category_id` 
  `address` 
...
* 보통 is 명칭을 앞에 사용하면 BOOLEAN  값을 의미 sql, java, javaSeript, python 등 어떤 언어에서든
...
SELECT customer_name, 
	...
	substring(email,locate('@', email)-1) as '이메일 도메인'
FROM CUSTOMERS;

-- 문제 5
-- CUSTOMERS 테이블에서 고객명, 이메일 아이디, 이메일 도메인을 각각 분리하여 조회하시오.
SELECT customer_name, substring(email,locate('@', email)) as '이메일 도메인'
FROM CUSTOMERS;
...
SELECT store_name, replace(store_name,'점','store') AS '변경된 점포명'
FROM STORES;

-- regexp_raplace 정규식을 이용해서 특정 명칭 변경 
-- ㅇㅇ$ : ㅇㅇ단어 마지막으로 조회되는 단어 만 변경
-- ^ㅇㅇ : ㅇㅇ단어 시작으로 조회되는 단어만 변경
SELECT store_name, regexp_replace(store_name,'점$','store') AS '변경된 점포명'
FROM STORES;
...
-- 문제 9
-- ORDERS 테이블에서 총 가격의 절댓값을 구하여 주문번호, 총가격, 절댓값(ABS)을 조회하시오.
