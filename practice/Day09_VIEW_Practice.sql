...
CREATE TABLE IF NOT EXISTS category (
    category_id INT AUTO_INCREMENT PRIMARY KEY,              -- 자동 증가 기본키
    category_name VARCHAR(50) NOT NULL UNIQUE,               -- 카테고리명 (NULL 불가, 중복 불가)
    description TEXT,                                        -- 설명
    is_active BOOLEAN DEFAULT TRUE,                          -- 활성 상태 (기본값 TRUE)
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP           -- 등록 시간 (기본값 현재시간)
);
...
CREATE TABLE IF NOT EXISTS customer (
    customer_id INT AUTO_INCREMENT PRIMARY KEY,          -- 자동 증가 기본키
    username VARCHAR(30) NOT NULL UNIQUE,                -- 사용자명 (NULL 불가, 중복 불가)
    email VARCHAR(100) NOT NULL UNIQUE,                  -- 이메일 (NULL 불가, 중복 불가)
    password VARCHAR(255) NOT NULL,                      -- 비밀번호
    full_name VARCHAR(50) NOT NULL,                      -- 실명
    phone VARCHAR(20),                                   -- 전화번호
    birth_date DATE,                                     -- 생년월일
    gender ENUM('M', 'F', 'OTHER'),                      -- 성별
    point INT DEFAULT 0 CHECK (point >= 0),              -- 적립금 (0 이상)
    grade VARCHAR(20) DEFAULT 'BRONZE',                  -- 등급 (기본값 BRONZE)
...
CREATE TABLE IF NOT EXISTS product (
    product_id INT AUTO_INCREMENT PRIMARY KEY,               -- 자동 증가 기본키
    product_name VARCHAR(100) NOT NULL,                      -- 상품명 (NULL 불가)
    category_id INT,                                         -- 카테고리 ID (외래키)
    price INT NOT NULL CHECK (price >= 0),                   -- 가격 (0 이상)
    discount_rate DECIMAL(5,2) DEFAULT 0.00 
        CHECK (discount_rate >= 0 AND discount_rate <= 100), -- 할인율 (0~100 사이)
    stock_quantity INT DEFAULT 0 CHECK (stock_quantity >= 0),-- 재고수량 (0 이상)
    description TEXT,                                        -- 상품설명
    brand VARCHAR(50),                                       -- 브랜드명
    weight DECIMAL(8,2),                                     -- 무게
    status ENUM('AVAILABLE','OUT_OF_STOCK','DISCONTINUED') 
        DEFAULT 'AVAILABLE',                                -- 상태
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,          -- 등록시간
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP 
        ON UPDATE CURRENT_TIMESTAMP,                         -- 수정시간 자동 업데이트
...
(username, email, password, full_name, phone, birth_date, gender, point, grade)
VALUES
...
*/
INSERT INTO product 
(product_name, category_id, price, discount_rate, stock_quantity, description, brand, weight, status)
VALUES
...
SELECT 
    ...
    price,
    discount_rate,
    price * (1 - discount_rate / 100) AS discounted_price
FROM product;
...
/* 
-- JOIN ON 
	양쪽 테이블에 모두 데이터가 존재하는 경우 만 보여줌
    상품이 존재하는 카테고리 정보 조회

-- LEFT JOIN ON
   왼쪽 테이블의 데이터는 조건의 일치 여부와 상관없이 모든 데이터를 가져와서 조회 
   오르쪽 테이블에 일치하는 데이터가 없으면 해당 컬럼은 NULL 표시 
