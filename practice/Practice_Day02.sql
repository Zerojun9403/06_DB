...
SELECT CONCAT(department_name, '(', capacity, ')') AS 학과정보
FROM DEPARTMENT;

-- 문제 6
-- 현재 날짜에서 7일 후, 30일 후, 365일 후를 조회하시오.
SELECT NOW() + interval 7 day as 7일후,
		NOW() + interval 1 MONTH as 30일후, 
        NOW() + interval 1 year as 365일후;
-- 문제 7
