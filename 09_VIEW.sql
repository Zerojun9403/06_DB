...
/*
 ...
 -- SELECT 문 의 실행 결과(RESULT SET)를 저장하는 객체
 
 -- VIEW 사용목적
 복잡한 SELECT 문을 숩게 재사용하기 위해 
 테이블의 진짜 모습을 감출 수 있어 보안상 유리
 
 VIEW 사용 시 주의사항
 가상의 테이블(실체X)이기 떄문에 ALTER VIEW 구문으로만 수정 가능
 VIEW 를 이용한 DML(INSERT, UPDATEM DELETE)를 사용가능 한경우도 있지만 제약이 많이 따르기 때문에
		SELECT 용도로 대부분 사용
